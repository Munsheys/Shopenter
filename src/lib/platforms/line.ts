...
/** Push a plain text admin alert to the merchant's own LINE account */
export async function pushAdminAlert(token: string, adminLineId: string, text: string): Promise<void> {
  if (!token || !adminLineId || !text) return;
  try {
    ...
    console.error('[pushAdminAlert]', err);
  }
}

/** Mark a user's chat as read in LINE (requires a Verified/Premium OA) */
export async function markLineMessagesAsRead(token: string, userId: string): Promise<void> {
  if (!token || !userId) return;
  try {
    await fetch('https://api.line.me/v2/bot/message/markAsRead', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
      body: JSON.stringify({ chat: { type: 'user', userId } }),
    });
  } catch (err) {
    console.error('[markLineMessagesAsRead]', err);
  }
}

/** PlatformAdapter implementation for LINE */
export const lineAdapter: PlatformAdapter = {
  sendMessage: sendLineMessage,
  sendRichMessage: sendFlexMessage,
};
