...
export default function NumberStepper({
  value, onChange, min, max, step = 1, isDark, disabled = false, size = 'md',
}: {
  ...
  return (
    <div className={`flex items-center ${h} rounded-xl border overflow-hidden ${isDark ? 'bg-[#0f1117] border-[#2a2f45]' : 'bg-white border-slate-200'}`}>
      <button type="button" disabled={disabled || (min !== undefined && value <= min)}
        onClick={() => onChange(clamp(value - step))} className={btnCls}>−</button>
