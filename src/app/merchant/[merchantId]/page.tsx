...
import { ShoppingBag, ChevronLeft, Plus, Minus, Trash2, User, Search, X, CheckCircle, ArrowRight, Package, SlidersHorizontal, ArrowUpDown, ChevronRight } from 'lucide-react';
...
export default function MerchantStorefront() {
  ...
  useEffect(() => {
    ...
    if (cached) {
      ...
      } catch (e) { localStorage.removeItem(`liff_profile_${merchantId}`); }
    } else if (hasParams) {
    ...
    const initShop = async () => {
      try {
        const infoRes = await fetch(`/api/storefront/${merchantId}/shop-info`, { cache: 'no-store' });
        if (!infoRes.ok) throw new Error('Failed to load shop info');
            ...
            if (liff.isLoggedIn()) {
              try {
                ...
                if (token) {
                  ...
                  setCustomer(p);
                  localStorage.setItem(`liff_profile_${merchantId}`, JSON.stringify(p));
                  setAuthStatus('logged_in');
  ...
  const handleConfirmOrder = async () => {
    ...
    try {
      const res = await fetch('/api/orders', {
        ...
        body: JSON.stringify({
          merchantId, // SaaS: tie to current merchant
  ...
  return (
    <div className={cn("min-h-screen text-[#1a1d2e] font-sans selection:bg-[#d4af37]/20", shopInfo.branding?.theme === 'dark' ? "bg-[#0a0d14] text-white" : "bg-[#fdfbf7]")}>
      {/* LUXURY HEADER */}
      <header className={cn("sticky top-0 z-50 backdrop-blur-xl border-b border-[#1a1d2e]/5 px-4 sm:px-8 py-3 sm:py-4", shopInfo.branding?.theme === 'dark' ? "bg-[#0a0d14]/90 border-white/5" : "bg-[#fdfbf7]/90")}>
        <div className="flex items-center justify-between max-w-7xl mx-auto">
          ...
          <div className="flex flex-col items-center cursor-pointer" onClick={() => setView('home')}>
             <h1 className={cn("text-xl sm:text-2xl lg:text-3xl font-serif font-black tracking-tight italic", shopInfo.branding?.theme === 'dark' ? "text-white" : "text-[#1a1d2e]")}>{shopInfo.name}</h1>
          </div>
      ...
      {/* Same logic as Shop component but scoped to merchantId */}
      {view === 'home' && (
        <main className="max-w-7xl mx-auto px-4 sm:px-8 pt-8 sm:pt-12 pb-32">
          {/* ... (Home view logic similar to src/app/shop/page.tsx) */}
          <div className="mb-10 lg:mb-16">
            <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-8 mb-10">
              <div className="text-center lg:text-left">
                <div className="flex items-center justify-center lg:justify-start gap-2 mb-2">
                  <h2 className={cn("text-4xl sm:text-6xl font-serif font-black tracking-tight italic", shopInfo.branding?.theme === 'dark' ? "text-white" : "text-[#1a1d2e]")}>Shop</h2>
                  {activeBrand !== 'All' && (
                ...
                <p className="text-[#8b92ad] text-sm font-medium">Curated boutique selection</p>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4 w-full lg:w-auto">
                <div className="relative flex-1 sm:w-[300px] lg:w-[350px] group">
                  ...
                  <input
                    ...
                    className={cn("w-full border rounded-full sm:rounded-2xl pl-12 pr-5 py-3.5 text-sm shadow-sm focus:border-[#d4af37] outline-none transition-all", shopInfo.branding?.theme === 'dark' ? "bg-[#161925] border-white/5 text-white placeholder-white/20" : "bg-white border-[#1a1d2e]/10 text-[#1a1d2e] placeholder-[#1a1d2e]/20")}
                  />
                </div>
                ...
                <div className="relative group min-w-[180px]">
                  ...
                  <select
                    ...
                    className={cn("w-full border rounded-full sm:rounded-2xl pl-12 pr-10 py-3.5 text-sm appearance-none shadow-sm focus:border-[#d4af37] outline-none transition-all cursor-pointer font-bold", shopInfo.branding?.theme === 'dark' ? "bg-[#161925] border-white/5 text-white" : "bg-white border-[#1a1d2e]/10 text-[#1a1d2e]")}
                  >
            ...
            {/* ... rest of the grid from shop/page.tsx, with shopInfo.branding?.theme checks ... */}
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-x-6 gap-y-12">
              {filteredAndSorted.map((p: Product) => (
                ...
                >
                  <div className={cn("aspect-square w-full rounded-[32px] overflow-hidden mb-4 relative shadow-sm group-hover:shadow-2xl transition-all border border-transparent group-hover:border-[#d4af37]/20", shopInfo.branding?.theme === 'dark' ? "bg-white/5" : "bg-[#1a1d2e]/5")}>
                  ...
                  <div className="px-2">
                    <p className={cn("font-serif font-black text-xl leading-tight mb-1 group-hover:text-[#d4af37] transition-colors line-clamp-1", shopInfo.branding?.theme === 'dark' ? "text-white" : "text-[#1a1d2e]")}>{p.name}</p>
      ...
      {view === 'detail' && selectedProduct && (
        <div className={cn("max-w-7xl mx-auto min-h-screen animate-in fade-in slide-in-from-bottom-6 duration-700 pb-40", shopInfo.branding?.theme === 'dark' ? "bg-[#0a0d14]" : "bg-[#fdfbf7]")}>
          <div className={cn("sticky top-16 sm:top-20 z-40 backdrop-blur-md px-4 sm:px-8 py-4 flex items-center justify-between", shopInfo.branding?.theme === 'dark' ? "bg-[#0a0d14]/80" : "bg-[#fdfbf7]/80")}>
            <button onClick={() => setView('home')} className={cn("w-10 h-10 rounded-full flex items-center justify-center", shopInfo.branding?.theme === 'dark' ? "bg-white/5 text-white" : "bg-[#1a1d2e]/5 text-[#1a1d2e]")}><ChevronLeft size={20} /></button>
          </div>
          <div className="lg:flex lg:gap-16 lg:px-8 lg:pt-8">
            <div className="px-5 mb-10 lg:w-1/2 lg:flex lg:justify-end">
              <div className={cn("rounded-[48px] overflow-hidden w-full aspect-square lg:w-[400px] lg:h-[400px] shadow-2xl border", shopInfo.branding?.theme === 'dark' ? "bg-white/5 border-white/5" : "bg-[#1a1d2e]/5 border-[#1a1d2e]/5")}>
            ...
            <div className="px-8 lg:w-1/2 lg:flex lg:flex-col lg:justify-center">
              ...
              <h2 className={cn("text-4xl sm:text-6xl font-serif font-black mb-4 lg:mb-8 tracking-tight leading-none italic", shopInfo.branding?.theme === 'dark' ? "text-white" : "text-[#1a1d2e]")}>{selectedProduct.name}</h2>
              ...
              <div className="flex items-end gap-3 mb-10 lg:mb-16"><span className={cn("text-4xl sm:text-6xl font-black", shopInfo.branding?.theme === 'dark' ? "text-white" : "text-[#1a1d2e]")}>฿{(selectedVariant?.price ?? selectedProduct.price)?.toLocaleString()}</span><span className="text-xs font-bold text-[#8b92ad] mb-2 uppercase tracking-widest">Investment</span></div>
              
              <div className="flex items-center justify-between bg-[#1a1d2e]/5 rounded-3xl p-4 lg:p-6 mb-10 lg:mb-16 lg:max-w-md">
                <span className="text-[10px] font-black uppercase tracking-widest text-[#d4af37]">Quantity</span>
                <div className="flex items-center gap-6 sm:gap-10">
                  <button onClick={() => setQty(q => Math.max(1, q - 1))} className={cn("w-8 h-8 sm:w-12 sm:h-12 rounded-full border flex items-center justify-center transition-all", shopInfo.branding?.theme === 'dark' ? "border-white/10 text-white hover:bg-white/5" : "border-[#1a1d2e]/10 text-[#1a1d2e] hover:bg-white")}><Minus size={14} /></button>
                  <span className={cn("font-black text-lg sm:text-2xl", shopInfo.branding?.theme === 'dark' ? "text-white" : "text-[#1a1d2e]")}>{qty}</span>
                  <button onClick={() => setQty(q => q + 1)} className={cn("w-8 h-8 sm:w-12 sm:h-12 rounded-full border flex items-center justify-center transition-all", shopInfo.branding?.theme === 'dark' ? "border-white/10 text-white hover:bg-white/5" : "border-[#1a1d2e]/10 text-[#1a1d2e] hover:bg-white")}><Plus size={14} /></button>
                </div>
              </div>
              <button onClick={addToCart} className="w-full max-w-md bg-[#d4af37] text-[#1a1d2e] py-6 rounded-[32px] font-black text-lg shadow-2xl active:scale-95 transition-all flex items-center justify-center gap-4"><ShoppingBag size={24} />ADD TO BAG</button>
            </div>
          </div>
        </div>
      )}

      {/* Payment and Cart views similar to Detail, using merchantId for orders */}
      {view === 'payment' && currentOrder && (
        <div className="max-w-3xl mx-auto px-6 pt-20 pb-32 text-center animate-in zoom-in-95 duration-700">
          ...
          <h2 className={cn("text-3xl sm:text-5xl font-serif font-black mb-2 tracking-tight italic", shopInfo.branding?.theme === 'dark' ? "text-white" : "text-[#1a1d2e]")}>Acquisition Confirmed</h2>
          ...
          <button onClick={() => setView('home')} className={cn("w-full max-w-md mx-auto py-5 rounded-[24px] font-black text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-2", shopInfo.branding?.theme === 'dark' ? "bg-white/5 text-white" : "bg-[#1a1d2e]/5 text-[#1a1d2e]")}>Finish & Return <ArrowRight size={16} /></button>
        </div>
      )}
      ...
      {isCartOpen && (
        <div className="fixed inset-0 z-[100] flex flex-col justify-end lg:justify-center lg:items-center">
          ...
          <div className={cn("relative border-t lg:border rounded-t-[40px] lg:rounded-[48px] w-full lg:max-w-2xl max-h-[90vh] lg:h-[80vh] flex flex-col animate-in slide-in-from-bottom duration-500 lg:zoom-in-95 shadow-2xl", shopInfo.branding?.theme === 'dark' ? "bg-[#161925] border-white/5" : "bg-white border-[#1a1d2e]/5")}>
            <div className="flex justify-center pt-4 pb-2 lg:hidden"><div className="w-12 h-1.5 bg-white/10 rounded-full" /></div>
            <div className="flex items-center justify-between px-8 py-6 sm:py-8 border-b border-white/5"><h3 className={cn("text-2xl sm:text-4xl font-serif font-black italic", shopInfo.branding?.theme === 'dark' ? "text-white" : "text-[#1a1d2e]")}>Your Selection</h3><button onClick={() => setIsCartOpen(false)} className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-white/5 flex items-center justify-center text-white/40"><X size={20} /></button></div>
            <div className="overflow-y-auto flex-1 px-8 py-6 space-y-8">
              {cart.length === 0 ? <div className="text-center py-20 lg:py-40"><ShoppingBag size={64} className="mx-auto mb-4 text-white/5" /><p className="font-black text-white/20 uppercase tracking-widest text-xs">Collection is empty</p></div> : cart.map((item, idx) => (
                <div key={idx} className="flex gap-6 sm:gap-10 items-center text-left">
                  <div className="w-20 h-20 sm:w-28 sm:h-28 rounded-[20px] sm:rounded-[32px] overflow-hidden bg-white/5 flex-shrink-0 border border-white/5">{item.imageUrl ? <img src={item.imageUrl} alt={item.name} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-white/10"><Package size={24} /></div>}</div>
                  <div className="flex-1 min-w-0"><p className={cn("font-serif font-black text-lg sm:text-2xl truncate leading-none mb-1 sm:mb-2", shopInfo.branding?.theme === 'dark' ? "text-white" : "text-[#1a1d2e]")}>{item.name}</p>{item.variantLabel && <p className="text-[10px] sm:text-xs font-bold text-[#8b92ad] uppercase tracking-wider">{item.variantLabel}</p>}<p className="text-[#d4af37] font-black text-base sm:text-xl mt-1">฿{(item.price * item.qty).toLocaleString()}</p></div>
                  <div className="flex flex-col items-center gap-4"><div className="flex items-center gap-4 bg-white/5 rounded-2xl px-3 py-1.5 sm:px-4 sm:py-2 border border-white/5"><button onClick={() => setCart(cart.map((it, i) => i === idx ? { ...it, qty: Math.max(1, it.qty - 1) } : it))} className="text-white/40 hover:text-white"><Minus size={14} /></button><span className="text-sm sm:text-lg font-black text-white w-4 text-center">{item.qty}</span><button onClick={() => setCart(cart.map((it, i) => i === idx ? { ...it, qty: it.qty + 1 } : it))} className="text-white/40 hover:text-white"><Plus size={14} /></button></div><button onClick={() => setCart(cart.filter((_, i) => i !== idx))} className="text-red-400/40 hover:text-red-400 transition-colors"><Trash2 size={16} /></button></div>
                </div>
              ))}
            </div>
            {cart.length > 0 && (
              <div className={cn("px-8 pt-8 pb-12 border-t rounded-b-[48px]", shopInfo.branding?.theme === 'dark' ? "bg-[#161925] border-white/5" : "bg-[#fdfbf7] border-[#1a1d2e]/5")}>
                <div className="flex justify-between items-center mb-8"><span className="text-[#8b92ad] font-black uppercase tracking-widest text-[10px] sm:text-xs">Total Investment</span><span className={cn("font-black text-3xl sm:text-5xl tracking-tighter", shopInfo.branding?.theme === 'dark' ? "text-white" : "text-[#1a1d2e]")}>฿{cartTotal.toLocaleString()}</span></div>
                <button onClick={handleConfirmOrder} disabled={isOrdering} className="w-full bg-[#d4af37] text-[#1a1d2e] py-5 sm:py-6 rounded-[24px] sm:rounded-[32px] font-black text-base sm:text-lg shadow-2xl active:scale-95 transition-all flex items-center justify-center gap-3">{isOrdering ? <div className="w-5 h-5 border-2 border-[#1a1d2e]/20 border-t-[#1a1d2e] rounded-full animate-spin" /> : <>CONFIRM ACQUISITION <ArrowRight size={18} /></>}</button>
              </div>
            )}
          </div>
        </div>
