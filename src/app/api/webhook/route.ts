...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Route by matching signature against all merchant LINE secrets
    ...
    if (!matchedSettings) {
      // Fallback: env-level credentials (useful for initial setup / single dev merchant)
      ...
      if (envSecret) {
        ...
        if (signature !== expected) {
          console.warn('[Webhook] No matching merchant found for signature');
              ...
              if (imgRes.ok) {
                ...
                if (slipRes.ok) {
                  ...
                  if (slipData.success && slipData.data?.amount) {
                    ...
                    if (toMark.length > 0) {
                      await Order.updateMany({ _id: { $in: toMark.map(o => o._id) } }, { $set: { status: 'paid' } });
                      const combinedProducts = toMark.map(o => `${(o.quantity || 1) > 1 ? `${o.quantity}x ` : ''}${o.product?.replace(/^\d+x\s/, '')}`).join(', ');
