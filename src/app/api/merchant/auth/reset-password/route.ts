...
export async function POST(req: NextRequest) {
  try {
    ...
    const merchant = await MerchantRepo.findByValidPasswordResetToken(tokenHash);
    ...
    await logAudit({ merchantId: merchant.id, action: 'password_reset_completed', resource: 'merchant', status: 'success' }, req);
    ...
    const jwtToken = signMerchantToken({ merchantId: merchant.id, email: merchant.email });
