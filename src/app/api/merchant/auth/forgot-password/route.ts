...
export async function POST(req: NextRequest) {
  try {
    ...
    await dbConnect();
    const merchant = await Merchant.findOne({ email: validation.data.email.toLowerCase().trim() });
    ...
    merchant.passwordResetTokenHash = tokenHash;
    merchant.passwordResetExpiresAt = new Date(Date.now() + RESET_TOKEN_TTL_MS);
    await merchant.save();
    ...
    await logAudit({ merchantId: merchant._id.toString(), action: 'password_reset_requested', resource: 'merchant', status: 'success' }, req);
