...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Prepare Flex Message for QR
    // The image URL will point back to our /api/qr route
    // We must use the absolute origin for the image URL
    const origin = req.headers.get('origin') || `https://${req.headers.get('host')}`;
    const qrUrl = `${origin}/api/qr?amount=${order.soldTHB}&ref=${order._id}`;
      ...
      contents: {
        ...
        body: {
          ...
          contents: [
            ...
            {
              type: "text",
              text: `฿${(order.soldTHB || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}`,
              weight: "bold",
              size: "xl",
              color: "#FF334B",
              margin: "md"
