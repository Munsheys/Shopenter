...
export default function Shop() {
  ...
  // Load shop info and init LIFF with "Bulletproof Handshake"
  useEffect(() => {
    // 1. Detect login callback parameters in BOTH query and hash
      ...
      try {
        ...
        if (data.liffId && !liffLock.current) {
          ...
          try {
            ...
            // Bulletproof Init: Always explicitly define the origin
            await liff.init({ 
              liffId: data.liffId,
            ...
            if (liff.isLoggedIn()) {
              ...
              // Only auto-login if in-app and NOT already trying to process a callback
              if (liff.isInClient() && !hasParams) {
                liff.login({ redirectUri: window.location.origin + '/shop' });
              }
            }
          } catch (liffErr) {
            console.error("LIFF Init Handshake Failed:", liffErr);
            setAuthStatus('guest');
          }
        }
      } catch (err) {
        console.error("Global Shop Init Failed:", err);
      }
    };
    ...
    <div className="min-h-screen bg-[#fdfbf7] text-[#1a1d2e] font-sans selection:bg-[#d4af37]/20">
          ...
          <div className="flex items-center gap-3 min-w-[120px] lg:min-w-[200px]">
            ...
            <div className="hidden sm:flex flex-col">
              <span className="text-[9px] font-black uppercase tracking-widest text-[#d4af37] leading-none mb-0.5">
                {authStatus === 'verifying' ? "Verifying Session" : customer ? "Verified Member" : "Guest Account"}
              </span>
              {authStatus === 'verifying' ? (
                <span className="text-xs font-bold text-[#1a1d2e] leading-none animate-pulse">Syncing...</span>
              ) : customer ? (
                <span className="text-xs font-bold text-[#1a1d2e] leading-none truncate max-w-[100px] lg:max-w-[150px]">
                  {customer.displayName}
                </span>
              ) : (
                <button onClick={manualLogin} className="text-xs font-bold text-[#1a1d2e] leading-none hover:text-[#d4af37] transition-colors">Sign in</button>
              )}
            </div>
          </div>
          ...
          <div className="lg:flex lg:gap-16 lg:px-8 lg:pt-8">
            ...
            <div className="px-8 lg:w-1/2 lg:px-0 lg:flex lg:flex-col lg:justify-center">
              ...
              <div className="flex items-end gap-3 mb-10 lg:mb-16">
                ...
                <span className="text-xs font-bold text-[#8b92ad] mb-2 uppercase tracking-widest">Investment</span>
              </div>
      ...
      {view === 'payment' && currentOrder && (
        <div className="max-w-3xl mx-auto px-6 pt-20 pb-32 text-center animate-in zoom-in-95 duration-700">
          ...
          <p className="text-[#8b92ad] text-sm sm:text-base font-medium mb-12">Thank you for your acquisition. Please complete the transfer below.</p>
